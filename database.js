...
async function seedData() {
  ...
  try {
    ...
    } else if (contentCount === 0 || contentCount === null) {
    ...
    if (prodError) {
    ...
    } else if (prodCount === 0 || prodCount === null) {
      ...
      const defaultProducts = [
        ...
        { name: 'तणनाशके', description: 'तण नियंत्रणासाठी उपयुक्त और विश्वासार्ह पर्याय.', image_path: 'images/herb.jpg', category: 'HERBICIDE' },
      ...
      const { error: insertProdError } = await supabase.from('products').insert(defaultProducts);
      if (insertProdError) console.error("Supabase Error (seeding products):", insertProdError.message);
    }
    ...
    if (brandError) {
      ...
      const { error: insertBrandError } = await supabase.from('brands').insert(defaultBrands);
      if (insertBrandError) console.error("Supabase Error (seeding brands):", insertBrandError.message);
    }
